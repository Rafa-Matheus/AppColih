...
export default withRouter(class extends Component {
    ...
    render() {
        return <React.Fragment>
            <TopBar user={this.props.user} count={this.state.notifications.length} />
            {this.state.notifications &&
                this.state.notifications.length > 0 &&
                this.getAccessLevel() < 4 &&
                <Category title="Notificações">
                    {this.state.notifications.map(item => {
                        if (item.nivel === "urgente")
                            return <ListItem
                                ...
                                title="Pendente"
                                action={() => { this.props.router.push(`/detalhes/${item.id}`) }} />
                    })}
                    {this.state.notifications.map(item => {
                        if (item.nivel !== "urgente")
                            return <ListItem
